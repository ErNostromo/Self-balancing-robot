...
void setup(){
  ...
  TCCR1A = 0;                                                               //Make sure that the TCCR2A register is set to zero
  TCCR1B = 0;                                                               //Make sure that the TCCR2A register is set to zero
  TIMSK1 |= (1 << OCIE1A);                                                  //Set the interupt enable bit OCIE2A in the TIMSK2 register
  TCCR1B |= (1 << CS11);                                                    //Set the CS21 bit in the TCCRB register to set the prescaler to 8
  OCR1A = 39;                                                               //The compare register is set to 39 => 20us / (1s / (16.000.000MHz / 8)) - 1
  TCCR1B |= (1 << WGM12);                                                   //Set counter 2 to CTC (clear timer on compare) mode
  ...
  pinMode(9, OUTPUT);                                                      //Configure digital poort 6 as output

  for(receive_counter = 0; receive_counter < 500; receive_counter++){       //Create 500 loops
    if(receive_counter % 15 == 0)digitalWrite(9, !digitalRead(9));        //Change the state of the LED every 15 loops to make the LED blink fast
...
ISR(TIMER1_COMPA_vect){
  ...
  */
  ...
  if(throttle_counter_right_motor > throttle_right_motor_memory){           //If the number of loops is larger then the throttle_right_motor_memory variable
    ...
    if(throttle_right_motor_memory < 0){                                    //If the throttle_right_motor_memory is negative
      //PORTD |= 0b00100000;                                                  //Set output 5 low to reverse the direction of the stepper controller
      PORTC |= 0b01000000;                                                  //Set output 5 low to reverse the direction of the stepper controller
      throttle_right_motor_memory *= -1;                                    //Invert the throttle_right_motor_memory variable
    }
    //else PORTD &= 0b11011111;                                               //Set output 5 high for a forward direction of the stepper motor
    else PORTC &= 0b10111111;                                               //Set output 5 high for a forward direction of the stepper motor
  }
  //else if(throttle_counter_right_motor == 1)PORTD |= 0b00010000;            //Set output 4 high to create a pulse for the stepper controller
  //else if(throttle_counter_right_motor == 2)PORTD &= 0b11101111;            //Set output 4 low because the pulse only has to last for 20us
